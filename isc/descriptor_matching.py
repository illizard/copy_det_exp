...
def query_iterator(xq):
    """ produces batches of progressively increasing sizes """ #d왜 점진적으로 배치사이즈를 2배씩 증가 시키는 거지 기하급수적으로 증가하는 꼴로
    nq = len(xq) #4991
    bs = 32
    i = 0
    while i < nq:
...
def search_with_capped_res(xq, xb, num_results, metric=faiss.METRIC_L2):
    ...
    # 230425
    print(f"exhaustive_search radius is {radius}")
    print(f"exhaustive_search len(lims) is {len(lims)}")
    print(f"exhaustive_search lims is {lims}")

    print(f"exhaustive_search len(dis) is {len(dis)}")
    print(f"exhaustive_search dis is {dis}")

    print(f"exhaustive_search  len(ids) is {len(ids)}")
    print(f"exhaustive_search ids is {ids}")
