...
def compute_metrics_track2(args):
    ...
    query_image_ids, xq = read_descriptors(args.query_descs) # xq = (4991, 768)
...
if __name__ == "__main__":
    ...
    aa("--gt_filepath", default='./list_files/public_ground_truth.csv', required=False, help="CSV file with ground truth")
    aa("--max_results", default=500_000, type=int, help="max number of accepted predictions")
    # aa("--max_results", default=10000000000, type=int, help="max number of accepted predictions")
    ...
    aa("--track2", default=True, action="store_true", help="perform track 2 evaluation (default is track 1)")
    aa("--db_descs", default=[f"./data/features_fullset/references_{i}_dino_base_patch16.hdf5" for i in range(20)], nargs='*', help="database descriptor file in HDF5 format")
    aa("--query_descs", nargs='*', help="query descriptor file in HDF5 format")
    aa("--max_dim", default=768, type=int, help="max number of accepted descriptor dimensions")
